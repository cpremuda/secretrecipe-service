...
var Kaiseki = function (applicationId, masterKey, sessionToken) {
...
Kaiseki.prototype = {
    API_BASE_URL : 'http://localhost:' + settings.server.port + settings.database.parse.server.serverURL + '/',
    ...
    updateObjects : function (className, updates, callback) {
        ...
        for (var i = 0; i < updates.length; i++) {
            ...
            requests.push({
                'method' : 'PUT',
                'path' : 'classes/' + className + '/' + update.objectId,
                'body' : update.data
            });
        }
        this._jsonRequest({
    ...
    uploadFile : function (filePath, fileName, callback) {
      ...
      } // http://stackoverflow.com/a/423385/246142
